...
function checkTime(i) {
  ...
  <div id="apDiv16">
	...
	$time=$_POST['time'];
	$count=0;
	$stmt_rpt = mysqli_prepare($conn, "SELECT * FROM report WHERE date=? AND patientID=?");
	mysqli_stmt_bind_param($stmt_rpt, "ss", $date1, $pid);
	mysqli_stmt_execute($stmt_rpt);
	$r = mysqli_stmt_get_result($stmt_rpt);
	...
	$stmt_rpt = mysqli_prepare($conn, "INSERT INTO report VALUES(?,?,?,?,?)");
	mysqli_stmt_bind_param($stmt_rpt, "sssss", $rid, $pid, $date1, $time, $docfile);
	$result = mysqli_stmt_execute($stmt_rpt);
