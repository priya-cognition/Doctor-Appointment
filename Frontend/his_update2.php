...
<div align="center">
  <?php
	...
	$date1=$_POST['date'];
	$time1=$_POST['time'];
	$stmt_upd = mysqli_prepare($conn, "UPDATE history SET details=?, date=?, time=? WHERE patientID=? AND doctorID=?");
	mysqli_stmt_bind_param($stmt_upd, "sssss", $detail, $date1, $time1, $pid1, $did);
	$qry = mysqli_stmt_execute($stmt_upd);
	if($qry)
	{
